...
use crate::{
    ImageBufferLayout, ImageBufferRole, ImageExecutionPlan, ImageOutputFormat, ImageTerminal,
};

/// Maximum deterministic compositing stages in one image plan.
pub const MAX_IMAGE_COMPOSITE_STAGES: usize = 32;
/// Maximum aggregate primary-plus-composite scratch bytes in one image graph.
pub const MAX_IMAGE_GRAPH_SCRATCH_BYTES: u64 = 512 * 1024 * 1024;

/// One image value available to a deterministic postprocess stage or output
/// route.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case", tag = "kind")]
pub enum ImageValueSource {
    /// Output of the primary diffusion or VAE-decode operation.
    Primary,
    /// Exact image input bound to the primary plan.
    Input {
        /// Caller-defined image input slot.
        slot: u16,
    },
    /// Output of an earlier compositing stage.
    Composite {
...
/// One deterministic postprocess operation.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case", tag = "kind")]
pub enum ImageCompositeOperation {
    /// Per-channel linear interpolation between two RGB8 images using one
    /// Gray8 mask.
    ///
    /// A mask byte of zero selects `base`; 255 selects `overlay`. Intermediate
    /// values use integer round-to-nearest:
    /// `(base * (255 - mask) + overlay * mask + 127) / 255`.
    MaskBlend {
        /// First RGB8 value.
        base: ImageValueSource,
        /// Second RGB8 value.
        overlay: ImageValueSource,
        /// Gray8 input slot.
        mask_slot: u16,
    },
}

/// One ordered deterministic compositing stage.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ImageCompositeStage {
    /// Zero-based stage index. Stages must be contiguous and declared in
    /// execution order.
    pub stage: u16,
    /// Exact compositing operation.
    pub operation: ImageCompositeOperation,
}

/// Transactional checkpoint mechanics around the primary diffusion
/// operation.
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ImageCheckpointPlan {
    /// Opaque checkpoint input slot restored before installed interventions at
    /// its bound post-step boundary.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub restore_from: Option<u16>,
    /// Post-step boundary captured before installed interventions.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub capture_after_step: Option<u32>,
}

/// Value routed into one caller-owned output allocation.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case", tag = "kind")]
pub enum ImageOutputSource {
    /// Exact image value.
    Image {
        /// Primary, input, or prior composite output.
        source: ImageValueSource,
    },
    /// Captured opaque checkpoint envelope.
    Checkpoint,
}

/// One ordered output route.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ImageOutputRoute {
    /// Value written to this route.
    pub source: ImageOutputSource,
    /// Exact caller-owned allocation metadata.
    pub buffer: BufferSpec,
    /// Exact output byte interpretation.
    pub layout: ImageBufferLayout,
}

/// Request-scope native cleanup behavior.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum ImageCleanupPolicy {
    /// Retain the known resident session for an explicit later cleanup.
    RetainSession,
    /// Confirm request-local cleanup and advance the session epoch before
    /// returning.
    ClearSession,
}

/// Version-two whole-image execution contract.
///
...
#[serde(deny_unknown_fields)]
pub struct ImageExecutionPlanV2 {
...
impl ImageExecutionPlanV2 {
    ...
    /// Returns the identity of the complete version-two plan.
    ///
    /// # Errors
    ///
    ...
    pub fn digest(&self) -> Result<Digest, CoreError> {
        self.validate()?;
        Digest::of_serializable("image-execution-plan-v2", self)
    }
}
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ImageOutputReceiptV2 {
...
/// Deterministic composite lineage.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ImageCompositeReceipt {
    /// Zero-based stage index.
    pub stage: u16,
    /// Exact first input bytes.
    pub base: Digest,
    /// Exact second input bytes.
    pub overlay: Digest,
    /// Exact mask bytes.
    pub mask: Digest,
    /// Exact RGB8 output bytes.
    pub output: Digest,
}

/// Observed request-scope cleanup outcome.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case", tag = "kind")]
pub enum ImageCleanupDisposition {
    /// The plan explicitly retained a known resident session.
    Retained,
    /// Cleanup was confirmed for the reported prior epoch.
    Confirmed {
        /// Session epoch invalidated by cleanup.
        cleared_epoch: u64,
    },
    /// Cancellation was observed before native or request-local state existed.
    NotRequired,
}

/// Deterministic whole-plan receipt for [`ImageExecutionPlanV2`].
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ImageExecutionReceiptV2 {
    /// Exact version-two plan identity.
    pub plan: Digest,
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub outputs: Vec<ImageOutputReceiptV2>,
...
impl ImageExecutionReceiptV2 {
    ...
    pub fn validate_for(&self, plan: &ImageExecutionPlanV2) -> Result<(), CoreError> {
        plan.validate()?;
    ...
    fn validate_cleanup(&self, plan: &ImageExecutionPlanV2) -> Result<(), CoreError> {
    ...
    ///
    /// Returns a validation or deterministic serialization error.
    pub fn digest_for(&self, plan: &ImageExecutionPlanV2) -> Result<Digest, CoreError> {
        self.validate_for(plan)?;
        Digest::of_serializable("image-execution-receipt-v2", self)
    }
}

fn maximum_checkpoint_lineage(plan: &ImageExecutionPlanV2) -> usize {
    usize::from(plan.checkpoint.restore_from.is_some())
        + usize::from(plan.checkpoint.capture_after_step.is_some()) * 2
}

/// Applies the version-two RGB8 mask-blend contract into caller-owned output.
///
/// Every length is validated before the first output byte is modified.
///
/// # Errors
///
/// Returns an error unless `base`, `overlay`, and `output` have equal RGB8
/// lengths and the mask contains one byte per pixel.
pub fn mask_blend_rgb8(
    base: &[u8],
    overlay: &[u8],
    mask: &[u8],
    output: &mut [u8],
) -> Result<ImageCompositeReceipt, CoreError> {
    if base.is_empty()
        ...
        || mask.len() != base.len() / 3
    {
        return Err(CoreError::invalid(
            "RGB8 mask blend",
            "image or mask lengths are inconsistent",
        ));
    }
    let base_digest = Digest::of_bytes("image-composite-rgb8-input-v1", base);
    let overlay_digest = Digest::of_bytes("image-composite-rgb8-input-v1", overlay);
    let mask_digest = Digest::of_bytes("image-composite-gray8-input-v1", mask);
    for (pixel_index, (pixel, mask)) in output
        .chunks_exact_mut(3)
        ...
        .enumerate()
    {
        ...
        for channel in 0..3 {
            let base = u16::from(base[offset + channel]);
            let overlay = u16::from(overlay[offset + channel]);
            let value = base * (255 - mask) + overlay * mask + 127;
            pixel[channel] = u8::try_from(value / 255).map_err(|_| {
                CoreError::invalid("RGB8 mask blend", "channel arithmetic overflowed")
            })?;
        }
    }
    let output_digest = Digest::of_bytes("image-composite-rgb8-output-v1", output);
    Ok(ImageCompositeReceipt {
        stage: 0,
        base: base_digest,
...
mod tests {
    ...
    fn version_two_round_trip_preserves_new_identity_domain() {
        ...
        let decoded: ImageExecutionPlanV2 = serde_json::from_slice(&encoded).unwrap();
        ...
        assert!(serde_json::from_value::<ImageExecutionPlanV2>(value).is_err());
    }
    ...
    fn mask_blend_is_exact_at_endpoints_and_midpoint() {
        ...
        let receipt = mask_blend_rgb8(&base, &overlay, &mask, &mut output).unwrap();
    ...
    fn mask_blend_rejects_before_write() {
        ...
        assert!(mask_blend_rgb8(&[0, 0, 0], &[1, 1], &[255], &mut output).is_err());
    ...
    fn prestart_cancellation_receipt_has_no_side_effects() {
        let plan = graph();
        let mut receipt = ImageExecutionReceiptV2 {
    ...
    fn cancellation_before_capture_can_leave_final_route_uninitialized() {
        ...
        let receipt = ImageExecutionReceiptV2 {
            ...
            outputs: vec![ImageOutputReceiptV2 {
                route: 0,
