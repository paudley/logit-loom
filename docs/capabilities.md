...
| Versioned stable-diffusion.cpp boundary | Companion ABI v1, image ABI v2, program ABI v3, model-block ABI v4, and `probe_companion` | Exact patch compilation is retained model-free evidence; Rust symbol/descriptor/callback/arena safety and per-step timing validation paths have model-free tests. Device probing and model execution remain opt-in. |
| Krea model-block residual scaling | `ModelBlockResidualScaleControlV1`, `model_block_residual_scale_schema_v1`, and `TensorSelector::ModelBlock` through `SdcppResidentProgram` | Exact fixed-width control/identity, bound, selector, and lowering tests plus full pinned companion compilation. The loaded Krea topology validates block indices. No semantic block role, image effect, or model-backed execution is established by ordinary tests. |
