...
| Structured projection and explicit retry | `StructuredProjectionPlanV1`, `StructuredProjectionController` | Model-free compiler/validator/feedback identity, hostile-bound, disposition, retry-lineage, callback-containment, candidate-order, and receipt tests plus adapter compilation; native projection execution is opt-in |
...
| Aggregate text-mechanics execution | `execute_text_mechanics`, `TextMechanicsRequest`, `TextMechanicsReceiptV2` | Model-free plan/checkpoint/steering/cleanup/cancellation accounting, checkpoint-scoped recurrent rollback allocation, strict adapter compilation, and ordinary/speculative composition; compatible-model execution remains opt-in |
...
| Versioned Krea inference schedule | `KreaDiscreteFlowScheduleV1` | Exact-bit and identity tests cover bounds, monotonicity, terminal zero, the one-over-steps final active boundary, and the published eight-step/`1.15` Turbo defaults; image efficacy remains consumer-owned |
...
| Resident image-program contract and driver | `ImageProgramPlanV1`, `ImageProgramReceiptV1`, `ImageProgramMeasurementsV1`, `ResidentImageProgramDriver`, `SdcppResidentProgram` | Default-built model-free tests cover multi-native branching and joining, direct VAE chaining, canvas-bound source/mask inputs, independently sized bounded RGB8/RGBA8 references with exact identity and adapter lowering, scheduled `LoRA` identities, authenticated checkpoints, scheduler-state controls/observations, exact-step Krea residual block controls with loaded-topology validation, typed RGB8/RGBA8/PNG/tensor/checkpoint/snapshot/receipt routes, canonical SSA producers, type/geometry rejection, liveness/release accounting, 2 GiB arena rejection, exact stage/cancellation prefixes, atomic publication, cleanup poisoning, and measurement separation. The exact companion patches compile; model-backed acceptance remains opt-in and pending. |
| Versioned stable-diffusion.cpp boundary | Companion ABI v2, image ABI v3, program ABI v3, model-block ABI v4, model-block application ABI v5, Krea activation ABI v6, exact native checkpoint continuation, and `probe_companion` | Exact patch compilation is retained model-free evidence; Rust symbol, descriptor, callback, continuation, arena, resident-input, resource-accounting, and cleanup paths have model-free tests. Device probing and model execution remain opt-in. |
| Krea model-block residual scaling | `ModelBlockResidualScaleControlV1`, `ModelBlockApplicationReceiptV1`, and `TensorSelector::ModelBlock` through `SdcppResidentProgram` | Exact fixed-width control/identity, bound, selector, lowering, transition-bitmap, graph-action, cleanup, and receipt-lineage tests plus full pinned companion compilation. The loaded Krea topology validates block indices. No semantic block role, image effect, or model-backed execution is established by ordinary tests. |
| Topology-bound Krea activation mechanics | `KreaActivationPlanV1`, `KreaActivationReceiptV1`, `KreaActivationMeasurementsV1`, and `SdcppResidentProgram::install_krea_activation` | Model-free topology, site/token/boundary/CFG selection, sealed and capture-SSA inputs, donor/vector/projection operations, zero-strength, exact callback evidence, resource peak, same-session reuse, stale-handle, idempotent cleanup, and poisoning tests plus full pinned ABI-v6 compilation. Compatible Krea execution remains opt-in; no semantic activation role or efficacy is established. |
| Create-new projected component transform | `ProjectedComponentPlanV1`, `project_safetensors_component_v1`, and `verify_projected_component_v1` | Model-free deterministic SafeTensors parsing and transformation tests cover both matrix sides, orthonormal basis validation, finite values, source/unselected-byte preservation, exact per-tensor lineage, complete recomputation, and rejection of malformed or incompatible inputs. Basis meaning and model behavior remain consumer-owned. |
...
| Request-local image LoRA cleanup | Image ABI v3 fixed LoRA stack | Native build verifies each requested adapter reaches at least one model tensor, then clears the stack before reusable returns; compatible model execution remains opt-in |
