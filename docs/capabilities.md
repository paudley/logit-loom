...
| Resident image-program contract | `ImageProgramPlanV1`, `ImageProgramReceiptV1`, `ImageProgramMeasurementsV1` | Default-built model-free tests cover multi-native branching and joining, direct VAE chaining, canonical SSA producers, type/geometry rejection, unused and forward references, mutable checkpoint fan-out, liveness/release accounting, 2 GiB arena rejection, stage-prefix receipts, cleanup uncertainty, and measurement separation. No native resident-program execution or model-backed acceptance is claimed. |
