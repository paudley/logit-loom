...
| Serializable whole-image execution | `ImageExecutionPlan`, `ImageExecutionReceipt` | Operation/input/output compatibility, exact layout, canonical schedule/LoRA ordering, plan identity, and terminal-boundary unit tests |
...
| Advanced whole-image operations | `AdvancedImageRequest` | Model-free tests for exact source/mask geometry, path-free request receipts, bounded references, fixed LoRA stacks, output accounting, and lifecycle classification; model execution is opt-in |
...
| Bulk-tokenizer substrate (unpublished, partial) | `logit-loom-tokenizer` | Identity, cancellation, scatter, batch, chunk, and bounded cache unit tests; the SIMD BPE kernel, dedicated pool, and differential qualification remain open |
