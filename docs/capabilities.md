...
| Resident image-program contract and driver | `ImageProgramPlanV1`, `ImageProgramReceiptV1`, `ImageProgramMeasurementsV1`, `ResidentImageProgramDriver` | Default-built model-free tests cover multi-native branching and joining, direct VAE chaining, canonical SSA producers, type/geometry rejection, unused and forward references, mutable checkpoint fan-out, liveness/release accounting, 2 GiB arena rejection, exact stage/cancellation prefixes, atomic output and self-receipt publication, corrupt materialization suppression, double release, cleanup poisoning, and measurement separation. The stable-diffusion.cpp version-three arena and model-backed acceptance remain pending. |
