...
| Versioned stable-diffusion.cpp boundary | Companion ABI v1, image ABI v2, program ABI v3, model-block ABI v4, model-block application ABI v5, and `probe_companion` | Exact patch compilation is retained model-free evidence; Rust symbol/descriptor/callback/arena safety and per-step timing validation paths have model-free tests. Device probing and model execution remain opt-in. |
